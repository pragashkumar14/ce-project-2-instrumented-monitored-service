...
# 1) Background traffic loop — runs for ~12 minutes, covers the whole talk
...
disown
echo "[1/3] Traffic loop started in background (~12 min)"

# 2) CPU stress — starts in 15s, runs for 2 minutes, spikes CPU + latency widgets
(
  sleep 15
  echo "CPU STRESS START: $(date -u +'%H:%M:%S UTC')" >> /home/ubuntu/app/demo.log
  stress-ng --cpu 2 --cpu-load 100 --timeout 120s --metrics-brief >> /home/ubuntu/app/demo.log 2>&1
  echo "CPU STRESS END: $(date -u +'%H:%M:%S UTC')" >> /home/ubuntu/app/demo.log
) > /dev/null 2>&1 &
disown
echo "[2/3] CPU stress scheduled (starts in 15s, runs 2 min)"
...
echo "  2:15  CPU stress ends"
echo "  ~5-10 min  Warning + Critical alarms fire -> 2 standard emails"
echo "  ~5-10 min  Lambda fires -> 1 enriched email with real log lines"
echo "  ~12 min    Traffic loop ends"
